...
void rpl_send_dao()
{
	...
	UINT i, route_count;
	IP_ROUTINGTABLE** route_list = iptable_get_table_by_type(DEVICE_NWLAYER(d)->ipRoutingTables, "RPL_DAO", &route_count);

	for (i = 0; i < route_count; i++)
	{
		IP_ROUTINGTABLE* route = route_list[i];
...
static IP_ROUTINGTABLE* check_is_route_is_present(PRPL_TARGET_OPTION target,
												  IP_ROUTINGTABLE** route_list,
												  UINT route_count)
{
	UINT i;
	for (i = 0; i < route_count; i++)
...
void rpl_process_dao_msg()
{
	...
	for (i = 0; i < target_count; i++)
	{
		...
		{ /* the route doesn't exist */
			...
			IP_ROUTINGTABLE* route = iptable_add((IP_ROUTINGTABLE**)&DEVICE_NWLAYER(d)->ipRoutingTables,
...
void rpl_dao_route_timeout()
{
	...
	for (i = 0; i < route_count; i++)
	{
		...
		if (route->update_time + RPL_DEFAULT_DAO_REMOVE_TIMEOUT <= time)
		{
			...
			iptable_delete_by_route((IP_ROUTINGTABLE**)&DEVICE_NWLAYER(d)->ipRoutingTables, route);
		}
		else
		{
