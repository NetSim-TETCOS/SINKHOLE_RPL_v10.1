...
static ptrMATCH get_match_table(IP_ROUTINGTABLE* table, NETSIM_IPAddress dest, bool isStatic)
{
	ptrMATCH l = NULL;
	ptrMATCH c = NULL;
	...
	while (table)
	{
		...
		else if (dest->type == table->networkDestination->type)
		{
			...
			network2 = table->networkDestination;
...
static void update_route_entry(ptrMATCH match, ptrIP_FORWARD_ROUTE route,NETSIM_IPAddress dest)
{
	...
	if (match)
	{
		IP_ROUTINGTABLE* table = match->table;
...
static ptrIP_FORWARD_ROUTE route_unicast(NetSim_PACKET* packet,
										 NETSIM_ID dev)
{
	IP_ROUTINGTABLE* routingTable = DEVICE_NWLAYER(dev)->ipRoutingTables;
...
static void change_table(NETSIM_ID d, IP_ROUTINGTABLE* table,
...
static void add_table(NETSIM_ID d, NETSIM_IPAddress dest, NETSIM_IPAddress mask,
					  NETSIM_IPAddress gateway, NETSIM_ID in, UINT metric)
{
	iptable_add(&IP_TABLE(d), dest, mask, 0, gateway, 1, &DEVICE_NWADDRESS(d, in), &in, metric, "STATIC");
}
...
void configure_static_ip_route(NETSIM_ID d, char* file)
{
	...
	while (fgets(input, BUFSIZ, fp))
	{
		...
		IP_ROUTINGTABLE* table = iptable_check(&IP_TABLE(d), dest, mask);
		if (table)
...
void pass_to_lower_layer(NetSim_PACKET* packet, ptrIP_FORWARD_ROUTE route, UINT c)
{
	...
	else
	{
		...
		packet->pstruMacData->szDestMac = (fn_NetSim_Stack_GetMacAddressFromIP(packet->pstruNetworkData->szNextHopIp));
