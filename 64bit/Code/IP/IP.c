...
static void add_default_ip_table_entry(NETSIM_ID d)
{
	...
	for (i = 1; i <= DEVICE(d)->nNumOfInterface; i++)
	{
		...
		ipRoutingTable->interfaceCount = 1;
		ipRoutingTable->Interface = calloc(1, sizeof* ipRoutingTable->Interface);
		ipRoutingTable->Interface[0] = ip;
		ipRoutingTable->nInterfaceId = calloc(1, sizeof* ipRoutingTable->nInterfaceId);
		ipRoutingTable->nInterfaceId[0] = i;
		ipRoutingTable->Metric = ONLINK_METRIC;
		ipRoutingTable->netMask = subnet;
		ipRoutingTable->networkDestination = IP_NETWORK_ADDRESS(ip,
																subnet,
																prefix);
		ipRoutingTable->prefix_len = prefix;

		ipRoutingTable->szType = _strdup("LOCAL");
		LIST_ADD(&(DEVICE_NWLAYER(d)->ipRoutingTables), ipRoutingTable, iptable_add_check);

		if (gateway && IP_COMPARE(ip, gateway))
		{
			ipRoutingTable = IPROUTINGTABLE_ALLOC();
			ipRoutingTable->gateway = gateway;
			ipRoutingTable->interfaceCount = 1;
			ipRoutingTable->Interface = calloc(1, sizeof* ipRoutingTable->Interface);
			ipRoutingTable->Interface[0] = ip;
			ipRoutingTable->nInterfaceId = calloc(1, sizeof* ipRoutingTable->nInterfaceId);
